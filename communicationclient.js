...
export default class {
  ...
  onMessage(topic, message) {
    ...
    const matching_topics = Object.keys(this.callbacks).filter(
      (registered_topic) => is_match(topic, registered_topic)
    );
  ...
  async awaitEvent(namespace, assetName, submodelName, eventName, timeout) {
    return this.awaitSingleMessage(
      `${namespace}/${assetName}/${submodelName}/${eventName}`,
      timeout
    );
  }
